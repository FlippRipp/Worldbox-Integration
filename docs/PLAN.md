...
2. **`on_turn_start(state, sdk)` / `on_turn_stopped(state, sdk, reason)`** — dispatched from the turn entry point, and with `reason="cancelled"` from the turn-cancel handler (`server.py:2679-2680`) or `reason="completed"` on normal turn completion. `reason="cancelled"` gives the toy module instant vibration halt when the user hits Stop (today only an idle watchdog would catch it).
...
Module-side hook duties: `on_stream_token` → `KeywordScanner.feed()` (sync, O(len(token)), exceptions never escape); `on_turn_start` → reset scanner turn state; `on_turn_stopped` → flush scanner tail, then if `reason=="cancelled"` (or `stop_on_turn_end` config) clear state + immediate device stop, else keep the latched state (idle watchdog backstops). Gating: core already filters dispatch by `__active_modules__`; `feed()` additionally checks the per-story `enabled` config.
...
- `VibeState` (latched — no durations, per user decision): a keyword match sets the current target `{strength 0-100, pattern}`; **last match wins** and holds until changed. Patterns are continuous waveforms evaluated each tick: `constant`, `pulse` (on/off ms from category config), `wave` (smooth sine between low % and full strength). `ramp` is dropped — it needs an endpoint, i.e. a duration. A rule with `strength: 0` is a **stop-word** (e.g. "finished", "calms", "drift off to sleep") that latches everything off. Output level = pattern(t) × strength × master cap; `clear()` = instant stop.
- Stop conditions (nothing expires on its own anymore): stop-word rules; `/toys stop` / STOP button / router; `on_turn_stopped(reason="cancelled")`; idle watchdog — no token and no manual action for `idle_timeout_s` (default 60) → fade to zero over ~2 s. Normal turn completion keeps the latched state (the story mood persists while you read/type) unless `stop_on_turn_end` is enabled in config. This requires the core `on_turn_stopped` hook to carry a `reason: "completed" | "cancelled"` argument.
- `DeviceManager.run()` 10 Hz loop: send on >2 % change or >0.9 s keepalive while level>0; explicit stop sent exactly once on reaching zero; `instant_stop()` out-of-band from widget/command/router. Task creation follows the wb_image_gen pattern (module-level task set + `asyncio.Lock`, lazily started on first running loop).
...
{ "lovense":  { "enabled": true,  "host": "", "port": 20010, "use_time_sec_safety": true },
  ...
  "master_cap_global": 100, "idle_timeout_s": 60, "stop_on_turn_end": false, "tick_hz": 10 }
...
7. `device_manager.py` + loop test with fake clients (change-threshold sends, keepalive, zero-once, kill-switch immediacy, watchdog).
